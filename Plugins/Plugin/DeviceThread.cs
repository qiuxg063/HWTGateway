...
namespace Plugin
{
    public class DeviceThread : IDisposable
    {
                        ...
                        if (Driver.IsConnected)
                        {
                                     ...
                                     .GroupBy(x => x.Alias))
                            {
                                ...
                                if (group.Any())
                                {
                                    ...
                                    //var triggerValues = triggerVariables.ToDictionary(x => x.Name, x => x.CookedValue);    //触发变量变为 true时才触发上传；

                                    var payLoadUnTrigger = new PayLoad() { Values = new Dictionary<string, object>() };

                                    //if (triggerValues.Values.Any(x => x is true) || !triggerVariables.Any())     //触发变量变为 true时才触发上传；
                                    // 任意类型变化即触发：只要触发变量与上次值不同，就允许发布
                                    if (!triggerVariables.Any() || triggerVariables.Any(HasTriggerChanged))        // 任意类型变化即触发：只要触发变量与上次值不同，就允许发布；
                                    {
        ...
        // 任意类型变化即触发：只要触发变量与上次值不同，就允许发布
        private static bool HasTriggerChanged(DeviceVariable variable)
        {
            return !AreValuesEqual(variable.CookedValues[0], variable.CookedValues[1]);
        }

        // 任意类型变化即触发：只要触发变量与上次值不同，就允许发布
        private static bool AreValuesEqual(object? current, object? previous)
        {
            ...
            try
            {
                ...
                var previousJson = JsonConvert.SerializeObject(previous);
