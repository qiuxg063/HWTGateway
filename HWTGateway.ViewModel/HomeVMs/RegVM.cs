...
namespace HWTGateway.ViewModel.HomeVMs
{
    public class RegVM : BaseVM
    {
        ...
        public bool DoReg()
        {
            ...
            {
                MSD.AddModelError("VerifyCode", Localizer["Login.ValidationFail"]);
                return false;
            }
        ...
        private bool ValidateVerifyCode()
        {
            ...
            var session = Wtm?.HttpContext?.Session;
            string verifyCode = null;
            if (session != null)
            {
                var bytes = session.TryGetValue("verify_code", out var value) ? value : null;
                if (bytes != null)
                {
                    verifyCode = System.Text.Encoding.UTF8.GetString(bytes);
                }
                session.Remove("verify_code");
            }

            return string.IsNullOrWhiteSpace(verifyCode) == false
                && string.IsNullOrWhiteSpace(VerifyCode) == false
                && string.Equals(verifyCode.Trim(), VerifyCode.Trim(), StringComparison.OrdinalIgnoreCase);
        }
    }
}