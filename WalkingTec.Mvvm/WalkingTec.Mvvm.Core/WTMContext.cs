    ...
    public class WTMContext : IDisposable
    {
        ...
        public LoginUserInfo DoLogin(string username, string password, string tenant)
        {
            ...
            else
            {
                ...
                var dbUser = BaseUserQuery.IgnoreQueryFilters().FirstOrDefault(x => x.ITCode == username && x.TenantCode == tenant && x.IsValid == true);
                if (dbUser == null)
                {
                    return null;
                }
                ...
                {
                    if (Utils.VerifyPassword(password, dbUser.Password) == false)
                    {
                        return null;
                    }

                    // 旧 MD5 密码登录成功后，自动升级为 PBKDF2
                    if (Utils.IsLegacyMd5Hash(dbUser.Password))
                    {
                        dbUser.Password = Utils.HashPassword(password);
                        _dc.SaveChanges();
                    }
                }
