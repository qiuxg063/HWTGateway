...
{
    /// <summary>
    /// 提供账户相关的API接口，包括登录、注册、登出、角色和分组查询等功能。
    /// </summary>
    ...
    public class AccountController : BaseApiController
    {
        [AllowAnonymous]
        [HttpPost("[action]")]
        public async Task<IActionResult> Login([FromForm] string account, [FromForm] string password, [FromForm] string tenant = null, [FromForm] bool rememberLogin = false, [FromForm] string verifyCode = null)
        {
            if (!ValidateVerifyCode(verifyCode))
            {
                return BadRequest(Localizer["Login.ValidationFail"].Value);
            }
        ...
        {
            if (!ValidateVerifyCode(loginInfo.VerifyCode))
            {
                ModelState.AddModelError("VerifyCode", Localizer["Login.ValidationFail"]);
        ...
        {
            if (!ValidateVerifyCode(regInfo.VerifyCode))
            {
                ModelState.AddModelError("VerifyCode", Localizer["Login.ValidationFail"]);
                return BadRequest(ModelState.GetErrorJson());
            }
            ...
            FrameworkUser user = new FrameworkUser
            {
                ...
                Name = regInfo.Name,
                Password = Utils.HashPassword(regInfo.Password),
                IsValid = true,
        ...
        private bool ValidateVerifyCode(string verifyCode)
        {
            ...
            var sessionVerifyCode = session.Get<string>("verify_code");
            session.Remove("verify_code");

            return string.IsNullOrWhiteSpace(sessionVerifyCode) == false
                && string.IsNullOrWhiteSpace(verifyCode) == false
                && string.Equals(sessionVerifyCode.Trim(), verifyCode.Trim(), StringComparison.OrdinalIgnoreCase);
        }
    }
